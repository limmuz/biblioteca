...
export default function HomePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setReadingList(meusLivros.filter(b => b.status === 'LENDO' || b.status === 'QUERO LER'));
  ...
  if (loading) return <div className={styles.page}><AppHeader /><main className={styles.main}><h2 style={{color: 'white', padding: '100px'}}>Carregando...</h2></main></div>;
      ...
      <main className={styles.main}>
            ...
            <div className={styles.carousel} aria-label="Últimas leituras">
              {recentlyRead.map((book) => (
                <div
                  ...
                  tabIndex={0}
                >
                  <img src={book.cover} alt={book.title} className={styles.carouselCover} />
        ...
        <div className={styles.bookGrid}>
          {readingList.length > 0 ? (
            readingList.map((book) => <BookCardMini key={book.id} book={book} />)
          ) : (
            <p style={{color: 'var(--color-sage)', padding: '20px'}}>Sua lista está vazia. Adicione livros!</p>
          )}
        </div>
  ...
  onClick={() => navigate('/listagem', { state: { query: 'trending' } })}
>
